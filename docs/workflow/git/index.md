...
- 下载相关
  ...
  - [使用 jsdelivr 加速 Github 仓库资源](https://github.com/LiXiuFengV1)
    ...
    steps:
      ...
      - name: Deploy
        ...
        with:
          ...
          # 指定仓库（根据需要设置）
          repository-name: LiXiuFengV1/Digital-Documents
          # GitHub Pages 读取的分支
          branch: gh-pages
          # 静态文件所在目录
          folder: dist
...
::: tip
以开发团队的 [Digital-Documents](https://github.com/LiXiuFengV1) 和 [daily-notes](http://github.com/LiXiuFengV1/daily-notes) 仓库为例
:::

1. `clone` 仓库 `mm-notes`（默认的 `remote` 为 `origin`）

```sh
git clone http://github.com/LiXiuFengV1/daily-notes
```

2. 添加远程仓库 `daily-notes`（`remote` 取名为 `daily-notes`）

```sh
git remote add daily-notes http://github.com/LiXiuFengV1/daily-notes
```
