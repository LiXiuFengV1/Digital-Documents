...
```ts
let name: string = 'Digital-Documents'
name = '数字化文档'
```
...
// 任意类型
const anyArray: Array<any> = ['egon', 18, {}]
```
...
```ts
const tuple: [string, number] = ['egon', 18]
...
padLeft('Hello world', 'egon ') // 'egon Hello world'
...
const instance = new Man('开发团队')
console.log(instance) // Man { name: '开发团队' }
instance.sayHello() // 'hello, 开发团队'
```
...
const man = new Person('egon')
man.name = '开发团队' // Error: 无法为“name”赋值，因为它是只读属性
```
...
const instance = new Man('开发团队',  18, '广东深圳')

console.log(instance.name)     // 开发团队
...
const instance = new Person('Dev Team')
console.log(instance.name) // 'Dev Team'
...
const instance = new Person('Dev Team')
instance.name = '开发团队' // Error: 无法为“name”赋值，因为它是只读属性
```
...
const instance = new Person('egon')
...
console.log(identity<string>('egon')) // 'egon'
```
...
console.log(person) // Person { name: 'egon' }
```
...
```ts
/* 对象 */
const obj = { name: 'Dev Team', age: 20 }
type Obj = typeof obj
