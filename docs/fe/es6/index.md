...
log('egon', 1, 2) // egon[1, 2]
log('egon', 1, 2, 3) // egon[1, 2, 3]
```
...
// 字符串会以字符数组的形式做合并
Object.assign({}, 'egon') // {0: 'm', 1: 'a', 2: 'o', 3: 'm', 4: 'a', 5: 'o'}
...
```js
const obj = { name: 'egon', age: 18 }
Object.keys(obj) // ['name', 'age']
Object.values(obj) //  ['egon', 18]
Object.entries(obj) // [['name', 'egon'], ['age', 18]]
```
...
```js
Object.fromEntries([['name', 'egon']]) // {name: 'egon'}

/* Map 转对象 */
const map = new Map([['name', 'egon']])
Object.fromEntries(map) // {name: 'egon'}

/* 将查询字符串转为对象 */
const params = 'name=egon&age=18'
Object.fromEntries(new URLSearchParams(params)) // {name: 'egon', age: '18'}
```
...
// util.js
/* 单个输出 */
export const name = 'egon'
export function log() {
  console.log(name)
}

/* 统一输出 */
const name = 'maoegonmao'
function log() {
  console.log(name)
}
export { name, log }
...
[Promise 实现](https://github.com/LiXiuFengV1) 简易实现、A+ 规范实现、原型方法、静态方法实现
