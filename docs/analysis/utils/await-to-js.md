...
function someAsyncFunction() {
  return new Promise(() => {
    throw new Error('egon - try-catch')
  })
}
...
try {
  ...
  console.log(err.message) // 'egon - try-catch'
}
```
...
function someAsyncFunction() {
  return new Promise(() => {
    throw new Error('egon')
  })
}

const [err, data] = await to(someAsyncFunction())
err && console.log(err.message) // 'egon'

/* 传递 errorExt 参数 */
const [err, data] = await to(someAsyncFunction(), { title: 'await-to-js' })
err && console.log(err.message, err.title) // 'egon' 'await-to-js'
```
